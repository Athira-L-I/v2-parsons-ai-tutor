  ...
  useEffect(() => {
    ...
    if (!parsonsWidget || problemChanged) {
      ...
      if (!isDependenciesLoaded()) {
        ...
        loadParsonsWidget().then(() => {
          if (window.ParsonsWidget) {
            initializeWidget();
          } else {
            console.error("ParsonsWidget is not available after loading scripts.");
          }
        }).catch(error => {
    ...
    try {
      ...
      if (feedback.success !== undefined) {
        ...
        if (!feedback.success) {
          ...
          const solution = parsonsWidget.getModifiedCode("#ul-" + sortableId).map((line: any) => {
            const indentSpaces = '    '.repeat(line.indent);
            return indentSpaces + line.code;
          });
