...
export default function DependencyTest() {
  ...
  const runTests = async () => {
    ...
    try {
      ...
      const requiredDeps = ['jquery', 'jqueryui', 'lodash', 'lis', 'parsonsWidget'];
      const missingDeps = requiredDeps.filter(dep => !DEPENDENCY_MANIFEST[dep]);
      
      if (missingDeps.length > 0) {
        addResult('Test 1.1', false, `Missing dependencies in manifest: ${missingDeps.join(', ')}`);
      } else {
        addResult('Test 1.1', true, 'Manifest contains all required dependencies');
      }
      ...
      if (invalidDeps.length > 0) {
        addResult('Test 1.2', false, `Dependencies with invalid structure: ${invalidDeps.join(', ')}`);
      } else {
        addResult('Test 1.2', true, 'All dependencies have valid structure');
      }
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      addResult('Test 1', false, `Dependency manifest test failed: ${errorMessage}`);
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        if (missingDeps.length > 0) {
          addResult('Test 3.1', false, `Missing window objects: ${missingDeps.join(', ')}`);
        } else {
        ...
        if (window.ParsonsWidget) {
          try {
            ...
            if (testElements.length > 0) {
              const ParsonsWidget = window.ParsonsWidget as new (options: Record<string, string>) => unknown;
              ...
              if (widget) {
                addResult('Test 3.2', true, 'ParsonsWidget instantiated successfully');
              }
            } else {
              addResult('Test 3.2', true, 'ParsonsWidget available (skipped instantiation)');
            }
          } catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error);
            addResult('Test 3.2', false, `ParsonsWidget instantiation failed: ${errorMessage}`);
          }
        }
      } else {
        addResult('Test 3', false, `Some dependencies failed to load: ${result.errors.join(', ')}`);
      }
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      addResult('Test 3', false, `Dependencies loading failed: ${errorMessage}`);
    }

    // Test 4: Test backward compatibility
    try {
      if (parsonsLoader.loadParsonsWidget && typeof parsonsLoader.loadParsonsWidget === 'function') {
        addResult('Test 4', true, 'Legacy loadParsonsWidget function exists');
        
        await parsonsLoader.loadParsonsWidget();
        
        if (window.ParsonsWidget) {
          addResult('Test 4.1', true, 'Legacy API loads ParsonsWidget successfully');
        } else {
          addResult('Test 4.1', false, 'Legacy API fails to load ParsonsWidget');
        }
        
        const isParsonsLoaded = parsonsLoader.isParsonsWidgetLoaded();
        if (isParsonsLoaded) {
          addResult('Test 4.2', true, 'isParsonsWidgetLoaded reports correctly');
        } else {
          addResult('Test 4.2', false, 'isParsonsWidgetLoaded reports incorrectly');
        }
        ...
        addResult('Test 4', false, 'Legacy loadParsonsWidget function not found');
      }
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      addResult('Test 4', false, `Backward compatibility test failed: ${errorMessage}`);
    }
  ...
  return (
    <div className="p-8">
      <h1 className="text-2xl font-bold mb-6">Dependency Management System Test</h1>
      ...
      <div className="mb-6">
        ...
        <div className="test-container border border-gray-300 p-4 rounded-md">
          <div id="test-sortable" className="min-h-[100px] bg-gray-100 mb-4 p-4 rounded-md">
            Sortable Container
          </div>
          <div id="test-trash" className="min-h-[100px] bg-gray-200 p-4 rounded-md">
            Trash Container
          </div>
        </div>
      </div>
